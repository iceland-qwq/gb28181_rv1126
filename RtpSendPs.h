...
{\
    ...
    public:
        ...
        explicit RtpSendPs(const char* rtpServerIp,int rtpServerPort,int localRtpPort,uint32_t SSRC_t);
        RtpSendPs() = delete;
        ~RtpSendPs();
    public:
        ...
        void stop();
        int set_filename(char * filename_);
    private:
        ...
        static void  SendStreamDataThread(void *arg);
        static void SendStreamDataThread_0(void *arg);
        static void SendDataThread(void *arg);
        static void SendDataDownload(void *arg);
    private:
        int mSockFd = -1;
