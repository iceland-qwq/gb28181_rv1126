...
extern bool thread0_start_play_ontime;
    ...
    RtpSendPs::RtpSendPs(const char* rtpServerIp,int rtpServerPort,int localRtpPort ,uint32_t SSRC_t):

    mRtpServerIp(rtpServerIp),mRtpServerPort(rtpServerPort),mLocalRtpPort(localRtpPort),SSRC(SSRC_t),mQuit(true){
...
//        addr.sin_addr.s_addr = inet_addr("localRtpIp");
    ...
    */
    ...
    void RtpSendPs::SendStreamDataThread(void *arg) {
        RtpSendPs *rtpSendPs = (RtpSendPs *)arg;

        Data_Info_s pPacker;
        pPacker.IFrame = 1;
        pPacker.u32Ssrc = rtpSendPs->SSRC;//10进制的ssrc
        pPacker.s64CurPts = 0;
        ...
        video_packet packet = sharedQueue.dequeue();

        // 转换为 unsigned char 指针使用
        if (packet.isIFrame){
        pPacker.IFrame = 1;
        }
	if (packet.size>0){
            
            rtpSendPs->gb28181_streampackageForH264(packet.buf.get()+19 ,packet.size, &pPacker, 0); //0 表示传递的是视频数据

            pPacker.s64CurPts += 3000;
            usleep(33 * 1000);//函数的休眠单位是微秒，这里休眠40毫秒
...
void RtpSendPs::SendStreamDataThread_0(void *arg) {
        RtpSendPs *rtpSendPs = (RtpSendPs *)arg;
        ...
        while (!rtpSendPs->mQuit) {
            ...
            video_packet packet = sharedQueue_0.dequeue();
    ...
    void RtpSendPs::SendDataThread(void *arg){
        ...
        FILE* fp = fopen(rtpSendPs->filename, "rb");
        if(!fp){
            LOGE("fopen error:%s",rtpSendPs->filename);
            return;
        }
        ...
        while (!rtpSendPs->mQuit) {
            ...
            if (size <= 0) {
                ...
                if (rtpSendPs->mSockFd!=-1) {
                    close(rtpSendPs->mSockFd);
                }
                break;
            }
            // 将h264码流读取到的一个一个nalu封装到ps并通过rtp推流
            ...
            pPacker.s64CurPts += 6000;


#ifndef WIN32
            usleep(65 * 1000);//函数的休眠单位是微秒，这里休眠40毫秒
#else
            //Sleep(33);//休眠40毫秒，考虑到我们准备的测试视频的fps是25，所以每一帧间隔40ms
#endif // !WIN32
...
void RtpSendPs::SendDataDownload(void *arg){
        RtpSendPs *rtpSendPs = (RtpSendPs *)arg;
        ...
        pPacker.u32Ssrc = rtpSendPs->SSRC;//10进制的ssrc
        pPacker.s64CurPts = 0;
        printf("filename is %s",rtpSendPs->filename);
        FILE* fp = fopen(rtpSendPs->filename, "rb");
        if(!fp){
            LOGE("fopen error:%s",rtpSendPs->filename);
            return;
    ...
    void RtpSendPs::start(int state)
    {

        if (state==start_play){
    	vencThread = new std::thread(&get_rv1126_nalu);
    	vencThread->native_handle();
            m_bRun= true;
        if(!mQuit){
            mThread = new std::thread(&RtpSendPs::SendStreamDataThread, this);
        ...
        else if (state == start_play_0) {
            m_bRun= true;
            if(!mQuit){
                mThread = new std::thread(&RtpSendPs::SendStreamDataThread_0, this);
                mThread->native_handle();
            }
        }
        else if (state == start_download) {

            m_bRun= true;
            if(!mQuit){
                mThread = new std::thread(&RtpSendPs::SendDataDownload, this);
                mThread->native_handle();
            }
        }
    ...
    void RtpSendPs::stop() {
        ...
        mQuit = true;
        m_bRun= false;
        thread0_start_play_ontime = false ;
        stop_media_cam();
        sharedQueue.clear();
        sharedQueue_0.clear();
    }
...
int RtpSendPs::set_filename(char *filename_) {
        filename = filename_;
        return 0;
    }
/*
int main() {
        RtpSendPs *my = new RtpSendPs("192.168.31.223",5000,20002,106151515);
        my->start();
        sleep(500);
    }
*/
